...
5. **收到挑战时的响应规则**：被挑战的 teammate 不要直接接受或直接反驳。先做三件事：（a）验证对方的质疑是否基于正确的上下文——对方可能缺少你掌握的信息；（b）如果质疑成立，修正你的结论并说明修正了什么；（c）如果质疑不成立，解释你的依据和对方可能遗漏的上下文。将最终结论通知 Lead。
6. **文件所有权**：不要修改不属于你的文件。如果你需要修改其他 teammate 负责的文件，通知 Lead 协调。避免 merge 冲突。
7. **面向决策者解释**：最终用户是不写代码的决策者。当你的结论涉及技术概念时，在关键决策点旁附上一句话的通俗解释——不是教学，而是确保决策者理解"选 A 或选 B"各自意味着什么。
